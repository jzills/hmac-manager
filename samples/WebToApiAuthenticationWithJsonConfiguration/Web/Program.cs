...
var apiUrl = builder.Configuration["ApiUrl"] ?? "http://localhost:5110";
...
// AddHmacManager takes a configuration section too. The signing side needs the
// same keys, algorithms and scheme headers — but not the ClaimTypes, which only
// matter to the end that verifies.
builder.Services.AddHmacManager(builder.Configuration.GetSection("HmacPolicies"));
